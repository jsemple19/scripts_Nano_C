...
#and convert it to a bed file with unique colours for each mother read and extract all
#pairwise hops within a mother read
...
#filename="data.tsv"
#filename="./TSV_NoAmp2-Pass_SpltAln_LAST-Trained_Stitched.tsv"
...
if (calcStats==TRUE) {
   rm(hops)
   allSNIDs<-unique(tsvDataManyFrag$Sample_NID)
   for (i in allSNIDs) {
      sameSNIDs<-tsvDataManyFrag[tsvDataManyFrag$Sample_NID==i,]
      allHops=combn(sameSNIDs$Frg_NID,2)
      for (j in 1:dim(allHops)[2]){
         lfrag=sameSNIDs[sameSNIDs$Frg_NID==allHops[1,j],]
         rfrag=sameSNIDs[sameSNIDs$Frg_NID==allHops[2,j],]
         lchr=lfrag$Chr
         rchr=rfrag$Chr
         #distance<-c(lfrag[c("Map_ExtBe","Map_ExtEn")]-rfrag[c("Map_ExtBe","Map_ExtEn")],
         #           lfrag[c("Map_ExtBe","Map_ExtEn")]-rev(rfrag[c("Map_ExtBe","Map_ExtEn")]))
         if (lchr==rchr) {
            interChr<-0
            if (rfrag$Map_ExtBe>=lfrag$Map_ExtEn) {
               distance<-rfrag$Map_ExtBe-lfrag$Map_ExtEn
               lpos=lfrag$Map_ExtEn
               rpos=rfrag$Map_ExtBe
            } else {
               distance<-lfrag$Map_ExtBe-rfrag$Map_ExtEn
               lpos=rfrag$Map_ExtEn
               rpos=lfrag$Map_ExtBe
            }
         } else {
            interChr<-1
            distance<-2.5e+07
   ...
   pdf(file=paste0(dataDir,"/plots/",fileBase,"_MH.pdf"),width=8,height=11,paper="a4")
   ...
   x<-barplot(distanceClass_freq, main=fileBase, ylab="frequency",
              xlab="sequential hop distance categories",ylim=c(0,max(distanceClass_freq)*1.1))
   text(x,distanceClass_freq*1.05,distanceClass_freq)
   dev.off()
   
   # write summary of statistics and hop data to files
   statsSummary<-list("fragmentsPerRead"=fragPR,"intraVSinterChr"=intChr,"multiHopDistance"=table(hops$distanceClass))
   saveRDS(statsSummary,file=paste0(dataDir,"/stats/",fileBase,"_MH.Rds"))
   write.csv(hops,file=paste0(dataDir,"/multiHops/",fileBase,"_MH.csv"),row.names=FALSE,quote=FALSE)
}
